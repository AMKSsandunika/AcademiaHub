...
if(isset($_POST['submit'])){
   ...
   if($_FILES['video']['size'] > $video_max_size){
      $message[] = 'Video size is too large! Please upload a video file smaller than 100 MB.';
   }else{

      if($thumb_size > 2000000){
         $message[] = 'Image size is too large!';
      }else{
         $add_playlist = $conn->prepare("INSERT INTO `content`(id, tutor_id, playlist_id, title, description, video, thumb, status, date) VALUES(?,?,?,?,?,?,?,?,?)");
         $add_playlist->execute([$id, $tutor_id, $playlist, $title, $description, $rename_video, $rename_thumb, $status, $date]);
